...
class Tools:
    ...
    async def compress(self, dl, out, log_msg):
        ...
        cmd = f'''{Var.FFMPEG} -hide_banner -loglevel quiet -progress """{_progress}""" -i """{dl}""" -metadata "Encoded By"="https://github.com/kaif-00z/AutoAnimeBot/" -preset ultrafast -c:v libx265 -crf {Var.CRF} -x265-params "pools=4:maxthreads=8:lookahead=32:bframes=4:ref=4:psy-rd=1.0:0.0:aq-mode=3" -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s?  """{out}""" -y'''
        process = await asyncio.create_subprocess_shell(
